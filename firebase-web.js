(function () {
  ...
  const DeerCampBilling = window.DeerCampBilling || {
    prices: {
      dcPlusMonthly: "price_1TXd6SRcr1GduppU9Cf2Qtln",
      dcPlusAnnual: "price_1TXd6RRcr1GduppUgXKo2qk9"
    },
    tiers: {
      DCF: "dcf",
      DC_PLUS: "dc_plus",
    ...
    async startCheckout(options = {}) {
      ...
      const session = await this.postJsonWithFallback([
        "https://createcheckoutsession-y2x2gdsiqq-uc.a.run.app",
    ...
    async manageBilling(options = {}) {
      ...
      const session = await this.postJsonWithFallback([
        "https://createbillingportalsession-y2x2gdsiqq-uc.a.run.app",
  ...
  const DeerCampAuth = window.DeerCampAuth || {
    ...
    async getStewardAccess(campId) {
      ...
      const endpoint = `https://us-central1-${projectId}.cloudfunctions.net/getStewardAccess`;
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({ campId: cleanCampId })
      });

      const payload = await response.json().catch(() => ({}));

      if (!response.ok) {
        throw new Error(payload?.error || "Could not verify Steward access.");
      }
