...
const DEFAULT_DASHBOARD_PATH = '/steward-dashboard.html';
...
function getEmailVariant(payload = {}) {
  const variant = normalizeValue(payload.emailVariant || payload.variant || '');
  return variant === 'return-link' ? 'return-link' : 'starter';
}
...
function buildStewardSetupLink(payload = {}) {
  ...
  if (variant === 'return-link') {
    const returnPath = getNormalizedPath(payload.returnPath || process.env.WELCOME_RETURN_PATH, DEFAULT_DASHBOARD_PATH);
    const returnUrl = new URL(returnPath, `${baseUrl}/`);
    appendIfPresent(returnUrl, 'campId', payload.campId);
...
function injectGreeting(html, text, recipientName) {
  ...
  const htmlNeedle = '{{greeting_block}}';
  const textNeedle = '{{greeting_text}}';

  return {
    html: html.replace(htmlNeedle, greetingHtml),
    text: text.replace(textNeedle, greetingText)
  };
}
...
function buildEmailCopy(payload = {}) {
  ...
  if (variant === 'return-link') {
    return {
      headline: 'Continue Your DeerCamp',
      introLine: 'We saved the progress you added to your camp.',
      detailLine: 'Use the link below to return to your Steward Dashboard and keep building without losing the content you already saved.',
      ctaLabel: 'Open Your Steward Dashboard',
      footerLabel: 'DeerCamp Steward Return Link Email',
      subject: campName ? `Continue Your DeerCamp — ${campName}` : 'Continue Your DeerCamp'
    };
  }
  ...
  return {
    ...
    detailLine: 'Your camp setup link is ready below. It includes the pre-filled details from your initial 5-minute DeerCamp builder form so you can pick up where you left off.',
    ctaLabel: 'Continue Building Your DeerCamp',
