...
export async function runCompare({ leftCampId, rightCampId, projectId }) {
  const { db, projectId: resolvedProjectId } = initializeFirestore(projectId);
  ...
  if (!leftSnapshot.exists) throw new Error(`Left camp does not exist: camps/${leftCampId}`);
  if (!rightSnapshot.exists) throw new Error(`Right camp does not exist: camps/${rightCampId}`);

  const comparison = compareObjects(leftSnapshot.data() || {}, rightSnapshot.data() || {});
  ...
  printFieldList(`ONLY IN LEFT CAMP (${leftCampId})`, comparison.onlyLeft);
  printFieldList(`ONLY IN RIGHT CAMP (${rightCampId})`, comparison.onlyRight);
  ...
  const leftReferences = await findTopLevelCampReferences(db, leftCampId, collections);
  const rightReferences = await findTopLevelCampReferences(db, rightCampId, collections);
